...
typedef struct {
  u8 state;               // Tracks the current system state of Grbl.
  u8 abort;               // System abort flag. Forces exit back to main loop for reset.
  u8 suspend;             // System suspend bitflag variable that manages holds, cancels, and safety door.
  u8 soft_limit;          // Tracks soft limit errors for the state machine. (boolean)
  u8 step_control;        // Governs the step segment generator depending on system state.
  u8 probe_succeeded;     // Tracks if last probing cycle was successful.
  u8 homing_axis_lock;    // Locks axes when limits engage. Used as an axis motion mask in the stepper ISR.
  u8 f_override;          // Feed rate override value in percent
  u8 r_override;          // Rapids override value in percent
  u8 spindle_speed_ovr;   // Spindle speed value in percent
  u8 spindle_stop_ovr;    // Tracks spindle stop override states
  u8 report_ovr_counter;  // Tracks when to add override data to status reports.
  u8 report_wco_counter;  // Tracks when to add work coordinate offset data to status reports.
  #ifdef ENABLE_PARKING_OVERRIDE_CONTROL
    u8 override_ctrl;     // Tracks override control states.
  #endif
...
extern s32 sys_position[N_AXIS];      // Real-time machine (aka home) position vector in steps.
extern s32 sys_probe_position[N_AXIS]; // Last probe position in machine coordinates and steps.

extern volatile u8 sys_probe_state;   // Probing state value.  Used to coordinate the probing cycle with stepper ISR.
extern volatile u8 sys_rt_exec_state;   // Global realtime executor bitflag variable for state management. See EXEC bitmasks.
extern volatile u8 sys_rt_exec_alarm;   // Global realtime executor bitflag variable for setting various alarms.
extern volatile u8 sys_rt_exec_motion_override; // Global realtime executor bitflag variable for motion-based overrides.
extern volatile u8 sys_rt_exec_accessory_override; // Global realtime executor bitflag variable for spindle/coolant overrides.
...
u8 system_control_get_state();

// Returns if safety door is open or closed, based on pin state.
u8 system_check_safety_door_ajar();

// Executes an internal system command, defined as a string starting with a '$'
u8 system_execute_line(char *line);
...
float system_convert_axis_steps_to_mpos(s32 *steps, s32 idx);

// Updates a machine 'position' array based on the 'step' array sent.
void system_convert_array_steps_to_mpos(float *position, s32 *steps);
...
u8 system_check_travel_limits(float *target);

// Special handlers for setting and clearing Grbl's real-time execution flags.
void system_set_exec_state_flag(u8 mask);
void system_clear_exec_state_flag(u8 mask);
void system_set_exec_alarm(u8 code);
void system_clear_exec_alarm();
void system_set_exec_motion_override_flag(u8 mask);
void system_set_exec_accessory_override_flag(u8 mask);
