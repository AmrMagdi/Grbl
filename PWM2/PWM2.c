...
void PWM2_voidInitialize(void)
{
	...
	/*channelA mode*/
/*	switch(PWM2_cfgArr[0].ChannelA_Mode)
	{
		case TOGGLE_OC2A:
			TCCR2B |=  (1<<WGM22);
			TCCR2A &= ~(1<<COM2A1);
...
*/
	...
	/*channelB mode*/
/*	switch(PWM2_cfgArr[0].ChannelB_Mode)
	{
		case NON_INVERT_OC2B:
			TCCR2A |=  (1<<COM2B1);
			TCCR2A &= ~(1<<COM2B0);
		break;
		case INVERT_OC2B:
