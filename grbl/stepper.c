...
void st_go_idle() {
	...
	if (((settings.stepper_idle_lock_time != 0xff) || sys_rt_exec_alarm
			|| sys.state == STATE_SLEEP) && sys.state != STATE_HOMING) {
...
ISR(TIMER1_COMPA_vect) {
	...
	if (st.exec_segment == NULL) {
		...
		if (segment_buffer_head != segment_buffer_tail) {
			...
			st.steps[X_AXIS] = st.exec_block->steps[X_AXIS]
					>> st.exec_segment->amass_level;
			st.steps[Y_AXIS] = st.exec_block->steps[Y_AXIS]
					>> st.exec_segment->amass_level;
			st.steps[Z_AXIS] = st.exec_block->steps[Z_AXIS]
					>> st.exec_segment->amass_level;
#endif
	...
	while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
		...
		if (pl_block == NULL) {
			...
			if (prep.recalculate_flag & PREP_FLAG_RECALCULATE) {
				...
				prep.req_mm_increment = REQ_MM_INCREMENT_SCALAR
						/ prep.step_per_mm;
				...
				if ((sys.step_control & STEP_CONTROL_EXECUTE_HOLD)
					...
					pl_block->entry_speed_sqr = prep.exit_speed
							* prep.exit_speed;
					prep.recalculate_flag &= ~(PREP_FLAG_DECEL_OVERRIDE);
				} else {
			...
			if (sys.step_control & STEP_CONTROL_EXECUTE_HOLD) { // [Forced Deceleration to Zero Velocity]
				...
				float decel_dist = pl_block->millimeters
						- inv_2_accel * pl_block->entry_speed_sqr;
				if (decel_dist < 0.0) {
					// Deceleration through entire planner block. End of feed hold is not in this block.
					prep.exit_speed = sqrt(
							pl_block->entry_speed_sqr
									- 2 * pl_block->acceleration
				...
				if (pl_block->entry_speed_sqr > nominal_speed_sqr) { // Only occurs during override reductions.
					...
					if (intersect_distance < pl_block->millimeters) { // Either trapezoid or triangle types
						...
						prep.decelerate_after = inv_2_accel
								* (nominal_speed_sqr - exit_speed_sqr);
						if (prep.decelerate_after < intersect_distance) { // Trapezoid type
							...
							prep.maximum_speed = sqrt(
									2.0 * pl_block->acceleration
											* intersect_distance
											+ exit_speed_sqr);
						}
		...
		do {
			...
			case RAMP_DECEL_OVERRIDE:
				...
				if (prep.current_speed - prep.maximum_speed <= speed_var) {
					...
					mm_remaining -= time_var
							* (prep.current_speed - 0.5 * speed_var);
			...
			case RAMP_CRUISE:
				...
				if (mm_var < prep.decelerate_after) { // End of cruise.
					...
					time_var = (mm_remaining - prep.decelerate_after)
							/ prep.maximum_speed;
		...
		uint32_t cycles = ceil(
				(TICKS_PER_MICROSECOND * 1000000 * 60) * inv_rate); // (cycles/step)
		...
		prep.dt_remainder = (n_steps_remaining - step_dist_remaining)
				* inv_rate;
...
float st_get_realtime_rate() {
	if (sys.state
			& (STATE_CYCLE | STATE_HOMING | STATE_HOLD | STATE_JOG
					| STATE_SAFETY_DOOR)) {
