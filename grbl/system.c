...
/*reset condition of uart*/
/*this function is used to set the condition that break the wait loop when UART tx buffer is full*/
void voidResetCondInUART(void)
{
...
/*global variable that will contain updated value of data reg*/
 u8 data;
...
void system_init()
{
	/* set condition of reset in uart*/
	/*pointer to function is used to break the (uart tx full buffer) wait loop*/
	UART_voidSetResetCondFn(voidResetCondInUART);

	/*get data register direct from uart */
	UART_voidGetDataReg(&data);

	/*set callback fn of RX uart*/
	UART_voidSetCallBackRx(UART_RX_ISR);
